...
public class FactoriesLocationsProblem implements IProblem<int[]> {
	...
	public FactoriesLocationsProblem(int n, int[][] distances, int[][] traffic) {
		this.n = n;
	...
	public double getValue(ISolution<int[]> solution) {
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				cost += traffic[i][j] * distances[locations[i]][locations[j]];
			}
		}
